...
def get_storage(storage, storage_name, storage_for):
    ...
    ] if storage_for in ("data", "hosted") else []
    if storage['type'] != 'fcp':
    ...
    if storage['type'] in FILE_STORAGE_TYPES:
        storage_details.append(
            "{storage_for}_storage_path=%s" % storage['path']
        )
    elif storage['type'] in BLOCK_STORAGE_TYPES:
        storage_details.append(
            "{storage_for}_storage_lun=%s" % storage['lun']
        )
    storage_details.append("")
...
def generate_hosted_engine_inventory(data, hypervisors_line):
    ...
    self_hosted_additional_hosts = "\n".join(
        hypervisors_line.format(
            **dict(h, nested=h.get('nested', False))
        )
...
def generate_inventory(data, inventory):
    ...
    rhel_hypervisors = "\n".join(
        hypervisors_line.format(**get_host_dict(h)) for h in
        get_rhel_hosts(data['hosts'])
    )
    rhvh_hypervisors = "\n".join(
        hypervisors_line.format(**get_host_dict(h)) for h in
        get_rhvh_hosts(data['hosts'])
    )
    content = "\n".join(content).format(
        engine=engine,
    ...
    with inventory:
        ...
        if hosted_engine:
            inventory.write(
                generate_hosted_engine_inventory(data, hypervisors_line)
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=argparse.FileType('w'), default="inventory",
    )
    parser.add_argument(
        '-v', '--env-vars',
...
def generate_env_vars(data, output):
    ...
    engine_os_release = RE_VERSION.match(
        data.get('engine_os')
    ).group('version')
    content = [
        ...
        'export VDS_RHEL=%s' % ','.join(
            h['address'] for h in get_rhel_hosts(data['hosts'])
        ),
        'export VDS_RHEL_OS_VER="%s"' % ' '.join(
            get_host_os_versions(rhel_hosts)
        ),
        'export VDS_RHVH=%s' % ','.join(
            h['address'] for h in get_rhvh_hosts(data['hosts'])
        ),
        'export VDS_RHVH_OS_VER="%s"' % ' '.join(
            ["%s:%s" % (host['address'], host['os']) for host in rhvh_hosts]
        ),
        'export BEAKER_HOSTS=%s' % ",".join(
            [
                h['address'] for h in data['hosts'] if h.get('managed_by') ==
                "beaker"
            ]
        ),
        'export ENGINE=%s' % data['engine_fqdn'],
        "export ENGINE_RHEL_RELEASE=%s" % engine_os_release,
        'export STORAGE_TYPES=%s' % ','.join(
            get_storage_types(all_storages_with_type)
        ),
        'export HOSTED_ENGINE=%s' % (
            True if data.get("hosted_engine_details", None) else False
        ),
        'export GLUSTER_ENV=%s' % extra_conf_options.get(
            "gluster_environment", False
        ),
    ]
    with output:
        output.write("\n".join(content))
...
def get_hosted_storage(storages):
    hosted_storage_params = [
        get_storage(storages[0], 'hosted_storage', 'hosted')
    ]
    for storage in storages:
